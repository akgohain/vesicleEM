...
def neuron_id_to_vesicle(conf, neuron_id, ratio=[1,4,4], opt='big', output_file=None, do_neuron=False):
    ...
    mask = None
    if do_neuron:
        mask = neuron_id_to_neuron(conf, neuron_id, ratio)        
    out = read_tile_h5_by_bbox(h5Name, bb[0], bb[1], bb[2], bb[3], bb[4], bb[5], \
                       zyx_sz, zz=[88,40,15], tile_step=ratio[1:], zstep=ratio[0], \
...
if __name__ == "__main__":
    ...
    if args.task == '0':        
        ...
        # python vesicle_mask.py -t neuron-vesicle -n 17
        neuron_id, neuron_name = neuron_to_id_name(conf, args.neuron)
        # zip -r vesicle_big_5_30-8-8.zip vesicle_big_5_30-8-8
        sn = f'{conf["result_folder"]}/vesicle_{args.vesicle}_{neuron_name}_{args.ratio[0]*30}-{args.ratio[1]*8}-{args.ratio[2]*8}' 
        seg = neuron_id_to_vesicle(conf, neuron_id, args.ratio, args.vesicle, sn)
        
        if args.ratio[1] == 1: # save into png for vast            
            mkdir(sn)
            # apply seg mask
            # neuron = neuron_id_to_neuron(conf, neuron_id, args.ratio)
            for z in range(seg.shape[0]):
                write_image(f'{sn}/seg_{z:04d}.png', seg[z], 'seg')
        else:# save into h5
            write_h5(f'{sn}.h5', seg)

    elif args.task == 'vast-process':
        # python vesicle_mask.py -t vast-process -if /data/projects/weilab/dataset/hydra/vesicle_pf/ -i KR4_8nm.h5,VAST_segmentation_metadata_KR4.txt -o KR4_sv_8nm.h5,KR4_lv_8nm.h5 -r 1,4,4
        ...
        if max(args.ratio) != 1:
            ...
            suffix = '-'.join([str(x) for x in args.ratio])
            write_h5(sv_file.replace('.h5',f'_{suffix}.h5'), out_sv)
            write_h5(lv_file.replace('.h5',f'_{suffix}.h5'), out_lv)
